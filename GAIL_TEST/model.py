...
from GAIL_TEST.expert_traj import ExpertTraj
...
class Actor(nn.Module):
    ...
    def forward(self, x):
        #z = torch.tensor(np.zeros([100, 1])).cuda().float()
        #x = torch.cat([x, z], 1)
        x = F.relu(self.l1(x))
        x = F.relu(self.l2(x))
...
class GAIL:
    ...
    def update(self, n_iter, batch_size=100):
        ...
        for i in range(n_iter):
            ...
            policy_losses.append(loss_policy.data.cpu().numpy())
            ...
            loss_actor.backward()
            agent_losses.append(loss_actor.data.cpu().numpy())
            self.optim_actor.step()
        return np.mean(policy_losses),np.mean(agent_losses)
            # log
